...
const IncentiveShare = require('../models/incentiveshare.model');
const IncentiveShareCtrl = require('.././controllers/incentiveshare.controller');
...
router.post('/', async (req, res) => {
    let result = IncentiveShareCtrl.insert(req.body);
    if (!isEmpty(result.errors)) {
        res.status(400).json(result.errors);
    }
    let newTherapy = new IncentiveShare(result.data);
    newTherapy
        .save()
...
router.put('/:therapyId', (req, res) => {
    if (mongodb.ObjectID.isValid(req.params.therapyId)) {
        ...
        IncentiveShare.findByIdAndUpdate(req.params.therapyId, result.data, { new: true }, function (err, therapy) {
            if (!therapy)
...
// @access  Private
router.get('/', (req, res) => {
    IncentiveShare.find((err, therapy) => {
        res.send(therapy);
    })
        .catch(err => {
...
router.delete('/:therapyId', (req, res) => {
    if (mongodb.ObjectID.isValid(req.params.therapyId)) {
        IncentiveShare.deleteOne({ _id: req.params.therapyId }, (err, therapy) => {
            if (err) throw err;
...
router.post('/import', function (req, res, next) {
    ...
    therapy.forEach(element => {
        ...
        let newTherapy = new IncentiveShare({
            bu: element.bu,
            bu_id: id,
...
router.get('/:sku_id', (req, res) => {
    IncentiveShare.find({ therapyline_id: req.params.sku_id }, (err, therapy) => {
        res.send(therapy);
    })
        .catch(err => {
            res.status(400).send("Update not possible");
