...
const IncentivePeriod = require('../models/incentiveperiod.model');
const IncentivePeriodCtrl = require('.././controllers/incentiveperiod.controller');
...
router.post('/', async (req, res) => {
    let result = IncentivePeriodCtrl.insert(req.body);
    if (!isEmpty(result.errors)) {
        res.status(400).json(result.errors);
    }
    let newTherapy = new IncentivePeriod(result.data);
    newTherapy
        .save()
...
router.put('/:therapyId', (req, res) => {
    if (mongodb.ObjectID.isValid(req.params.therapyId)) {
        ...
        IncentivePeriod.findByIdAndUpdate(req.params.therapyId, result.data, { new: true }, function (err, therapy) {
            if (!therapy)
...
// @access  Private
router.get('/', (req, res) => {
    IncentivePeriod.find((err, therapy) => {
        if(!therapy){
            res.status(404).send("No Data Avialable")
        }
        res.send(therapy);
    })
        .catch(err => {
...
router.delete('/:therapyId', (req, res) => {
    if (mongodb.ObjectID.isValid(req.params.therapyId)) {
        IncentivePeriod.deleteOne({ _id: req.params.therapyId }, (err, therapy) => {
            if (err) throw err;
...
router.post('/import', function (req, res, next) {
    ...
    therapy.forEach(element => {
        ...
        let newTherapy = new IncentivePeriod({
            bu: element.bu,
            bu_id: id,
...
router.get('/:sku_id', (req, res) => {
    IncentivePeriod.find({ therapyline_id: req.params.sku_id }, (err, therapy) => {
        if(!therapy){
