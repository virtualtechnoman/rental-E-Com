...
//Return Product of an order
router.put("/return", authorizePrivilege("ACCEPT_CUSTOMER_ORDER"), (req, res) => {
    let result = CustomerOrderController.verifyReturn(req.body);
    if (!isEmpty(result.errors)) {
        return res.status(400).json({ status: 400, errors: result.errors, data: null, message: "Fields required" });
    }
    if ((!mongodb.ObjectID.isValid(result.data.product)) || (!mongodb.ObjectID.isValid(result.data.order))) {
        return res.status(400).json({ status: 400, errors: true, data: null, message: "Invalid product or order id" });
    }
    CustomerOrder.findOneAndUpdate({ _id: result.data.order, "products.product": result.data.product }, { $set: { "products.$.returned": result.data.returned } }, { new: true })
    .populate({ path: "placed_by"}).populate({
        path: "products.product",
        populate: {
            path: "product attributes.attribute attributes.option category brand"
        }
    }).lean().exec().then(d => {
        res.json({ status: 200, data: d, errors: false, message: "Order updated successfully" });
    }).catch(e => {
        console.log(e);
...
//Return Product of an order
router.put("/depositrefund", authorizePrivilege("ACCEPT_CUSTOMER_ORDER"), (req, res) => {
    let result = CustomerOrderController.verifyRefund(req.body);
    if (!isEmpty(result.errors)) {
        return res.status(400).json({ status: 400, errors: result.errors, data: null, message: "Fields required" });
    }
    if ((!mongodb.ObjectID.isValid(result.data.product)) || (!mongodb.ObjectID.isValid(result.data.order))) {
    ...
    CustomerOrder.findOneAndUpdate({ _id: result.data.order, "products.product": result.data.product }, { $set: { "products.$.depositRefunded": result.data.depositRefunded } }, { new: true })
    .populate({ path: "placed_by"}).populate({
        path: "products.product",
        populate: {
            path: "product attributes.attribute attributes.option category brand"
