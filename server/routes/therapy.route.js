...
const therapyCtrl = require('.././controllers/therapy.controller');
...
router.post('/', async (req, res) => {
    let result = therapyCtrl.insert(req.body);
    if (!isEmpty(result.errors)) {
        res.status(400).json(result.errors);
    }
    let newTherapy = new Therapy(result.data);
    newTherapy
        .save()
...
router.put('/:therapyId', (req, res) => {
    if (mongodb.ObjectID.isValid(req.params.therapyId)) {
        ...
        Therapy.findByIdAndUpdate(req.params.therapyId, result.data, { new: true }, function (err, therapy) {
            if (!therapy)
...
router.delete('/:therapyId', (req, res) => {
    if (mongodb.ObjectID.isValid(req.params.therapyId)) {
        Therapy.deleteOne({ _id: req.params.therapyId }, (err, therapy) => {
            if (err) throw err;
