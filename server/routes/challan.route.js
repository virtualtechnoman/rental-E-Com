...
router.get("/", authorizePrivilege("GET_ALL_CHALLAN_OWN"), (req, res) => {
    ...
    Challan.find({ processing_unit_incharge: req.user._id }).populate("processing_unit_incharge dispatch_processing_unit products.product vehicle driver").exec().then(doc => {
        if (doc.length > 0)
...
router.put("/:id", (req, res) => {
    if (mongodb.ObjectID.isValid(req.params.id)) {
        ...
        res.status(400).json({ status: 400, errors: false, data: null, message: "Invalid order id" });
>>>>>>> 6b69aed73bfb310431135384ba1866af7a411464
    }
})
