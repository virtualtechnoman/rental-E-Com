...
router.post("/", authorizePrivilege("ADD_PRODUCT_TO_CART"), (req, res) => {
    let result = TicketController.verifyAdd(req.body);
    if (!isEmpty(result.errors))
        return res.status(400).json({ status: 400, errors: result.errors, data: null, message: "Fields required" });
    Ticket.findOneAndUpdate({ user: req.user._id }, { $push: { products: result.data } }, { upsert: true, new: true }).exec().then(d => {
        Ticket.aggregate([
            ...
            if (doc) {
                Ticket.populate(doc, { path: "products.product", populate: { path: "category created_by", select: "-password" } }, (err, doc) => {
                    if (err) {
                        console.log(err);
...
router.delete("/:id", authorizePrivilege("DELETE_PRODUCT_FROM_CART"), (req, res) => {
    if (!mongodb.ObjectId.isValid(req.params.id)) {
        ...
        Ticket.findOneAndUpdate({ user: req.user._id }, { $pull: { products: { product: req.params.id } } }, { new: true }, (err, doc) => {
            if (err) {
...
router.put("/:id", authorizePrivilege("UPDATE_QUANTITY_IN_CART"), (req, res) => {
    if (mongodb.ObjectID.isValid(req.params.id)) {
        ...
        let result = TicketController.verifyUpdate(req.body);
        if (!isEmpty(result.errors)) {
            return res.status(400).json({ status: 400, errors: false, data: null, message: result.errors });
        }
        Ticket.findOneAndUpdate({ user: req.user._id, 'products.product': req.params.id }, { $set: { 'products.$.quantity': req.body.quantity } }, { new: true }).populate('products.product').exec()
            .then(doc => {
                Ticket.aggregate([
                    { $match: { user: req.user._id } },
                    { $unwind: "$products" },
...
router.post("/placeorder", authorizePrivilege("PLACE_ORDER"), (req, res) => {
    Ticket.findOne({ user: req.user._id }).exec().then(cart => {
        Ticket.aggregate([
            { $match: { user: req.user._id } },
            { $unwind: "$products" },
            { $lookup: { "from": 'products', "localField": 'products.product', "foreignField": '_id', "as": 'prod' } },
